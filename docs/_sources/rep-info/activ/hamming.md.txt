...
Mettre en place le contexte et le principe de l'activité.
...
Les expéditeurs définissent le message, l'encode en positionnant les jetons sur les supports et envoient le message en passant par l'enseignant.  
L'enseignant joue le rôle du canal de transmission (quelque il soit) qui peut avoir des défaillances qui modifie **1 bit maximum** par mot envoyé.
...
### 4.6.2. Étape 2 - Sécuriser la transmission

#### Objectif

Trouver un moyen pour que le destinataire puisse détecter un problème lors de la transmission.
...
**Redondance (+4 bits)** On duplique le message et on transmet le tout, soit :
...
**Bit de parité (+1 bit)** On fait la somme du nombre de bits à 1 dans le message original puis on donne la valeur 0 au bit de parité lorsque cette somme est paire et la valeur 1 lorsque cette somme est impaire. On notera que, de cette façon, la somme des 5 bits est toujours paire.
...
**Bits de position (+2 bits)** Les 4 positions des bits du message peuvent être encodés sur 2 bits. À l'instar de l'encodage utilisé pour résoudre le problème des bouteilles empoisonnées, on pourrait calculer la parité des bits dont la représentation binaire de leur position ont 1 bit en commun.  
