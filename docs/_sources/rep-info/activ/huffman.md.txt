...
* Nom : Compression et codage de Huffman
* Durée : 1 période
* Thème : Représentation de l'information
* Objectifs d’apprentissage : Faire comprendre aux élèves le principe de base derrière la compression de données.
...
1. {ref}`Premier jeu<huffman.premierjeu>` (durée `à compléter`)

1. {ref}`Second jeu<huffman.secondjeu>` (durée `à compléter`)

1. {ref}`Et la compression, dans tout ça ?<huffman.compression>` (durée `à compléter`)

1. {ref}`Développements<huffman.developpements>` (durée `à compléter`)

1. {ref}`Autres algorithmes de compression<huffman.autres>` (durée `à compléter`)

1. {ref}`Conclusion<huffman.conclusion>` (durée `à compléter`)
