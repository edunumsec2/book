...
## Objectifs
L'objectif de cette activité est de faire se familiriser les élèves aves les portes de base **ET**, **OU** et **NON** en dehors d'un contexte trop «technique» avec un petit exemple suivi qui doit nous dire si oui ou non un chien donné correspond à une liste de critères.
...
### Deuxième partie: critères plus compliqués
  ...
  1. Les critères sont maintenant devenus plus complexes. Le chien doit maintenant remplir ces conditions:
     * Le chien est un chien doit être gentil;
...
### Troisième partie: encodage et décodage des races
  ...
  1. Expliquer aux élèces que l'entrée «labrador» de la partie précédente n'est pas très intéressante, car elle ne permet de modéliser qu'une seule race de chiens. Dans cette deuxième partie, on va se permettre d'utiliser deux bits pour représenter plusieurs races. Faire réfléchir les élèves à ceci: combien de races pourra-t-on au maximum représenter si on se permet d'utiliser deux entrées? La réponse est 4 et non pas 2.
  2. Dire aux élèves qu'on va donc s'intéresser, pour simplifier, à uniquement 4 races de chiens: border collie, berger allemand, husky et labrador. On décide de l'encodage suivant:
     ...
     On a donc maintenant besoin d'un décodeur: en utilisant les 2 bits d'entrées, il s'agit d'avoir un circuit qui va activer une seule des quatre sortie, celle correspondant à la race du chien représentée selon la table ci-dessus.
