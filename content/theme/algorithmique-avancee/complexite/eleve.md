...
```{admonition} Solutions des exercices 0 et 1
:class: note

La solution des exercices est donnée directement dans le texte qui suit l’énoncé.
...
print("L'algorithme linaire a été {0:.2f} fois plus lent.".format(time_algo_lin/time_algo_bin))
...
```{admonition} Solution des exercices 5 et 6
:class: note
...
name : fig-tri-rapide
---
Illustration du tri rapide sur le même set de données que celui utilisé pour illustrer les algorithmes de tri vus au chapitre précédent. L’élément pivot est le dernier élément des tableaux à trier.
```
...
Intéressons‑nous maintenant à la complexité de cet algorithme. A chaque étape (chaque ligne dans la figure ci-dessus), on compare tout au plus n éléments avec les éléments pivots. Mais combien d’étapes faut-il pour que cet algorithme se termine ? 
...
L’algorithme commence par une phase de division : on découpe le tableau en deux, jusqu’à arriver à uniquement des tableaux à 1 élément (voir la figure ci-dessous). Le nombre d’étapes nécessaires pour découper le tableau en tableaux à 1 élément en divisant toujours les tableaux en deux est `log(n)`.

La deuxième phase consiste à fusionner les petits tableaux. On commence par fusionner les éléments 1 à 1, dans un ordre qui dépend de leur taille. Il suffit d’assembler les deux éléments du plus petit au plus grand (voir la 2e ligne de la 2e figure ci-dessous).
...
name : fig-div-fus
---
Illustration de la première phase du tri fusion. A chaque étape le tableau est découpé en deux jusqu’à ce qu’il ne reste que des tableaux à 1 élément
```
