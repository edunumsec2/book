...
Ce chapitre traite de la question de l'architecture des ordinateurs, c'est à dire des multiples couches physiques qui rendent possibles des opérations numériques aussi complexes que celles qu'effectuent à chaque instant nos smartphones par exemple. 
...
```
Ce chapitre traite principalement des couches de bas niveau, puis remontera progressivement jusqu'aux couches logicielles de haut niveau. 
...
Les {glo}`codebinaire|nombres binaires`, au niveau le plus élémentaire, sont matérialisés par des <span commented>courants ou tensions électriques</span><!-- REVIEW/JPP: discussion courant vs tension? -->, qui traversent les circuits des ordinateurs. Mais pourquoi avoir choisi des 0 et des 1 comme alphabet ? Quel rapport avec l'électricité ? 

En informatique, l'utilisation d'un {glo}`codebinaire|code binaire`, n'est pas par hasard. Ce sont les deux signaux les plus élémentaires que l'on puisse transmettre avec l'électricité. Soit le courant passe, soit il ne passe pas. Ouvert ou fermé ; allumé ou éteint ; 1 ou 0. 
...
Dans le chapitre suivant, où la conception des circuits électroniques des ordinateurs est abordée, est évoquée d'abord la notion de **{glo}`portelogique|portes logiques`**. Ce sont des composants qui sont eux-mêmes constitués de plusieurs transistors. Réfléchir en termes de portes logiques permet de véritablement concevoir les circuits des ordinateurs qui vont manipuler les bits d'informations constituant nos données.
